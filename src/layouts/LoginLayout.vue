...
function checkAdminAddress(address: string) {
  return axios
    .get(backServerUrl + "/v1/checkAdmin", { params: { address: address } })
...
async function resetApp(store, data) {
  await data.client?.disconnect({topic: data.session.topic,reason: ERROR.USER_DISCONNECTED.format()});
...
export default {
  ...
  setup() {
    ...
    // 请求登陆
    const login = async () => {
      data.client = await Client.init({relayProvider: DEFAULT_RELAY_PROVIDER});
      subscribeToEvents();
      init();
    ...
    eventHub.$on("connect", async (metadata: AppMetadata) => {
      ...
      setTimeout(async () => {
        if (data.accounts.length === 0 && !store.state.account) {
          $q.dialog({component: WalletSelectDialog,})
    ...
    eventHub.$on("addressFailed", () => {
      showErrorDialogExe("使用管理员钱包登陆");
      resetApp(store, data);
    });
    // 签名完成开始登陆
    eventHub.$on("signSucess", () => {startLogin(router, data, store)});
    ...
    const showErrorDialogExe=(errorInfo:string)=>{
      showErrorDialog.value = true;
      showErrorMessage.value = errorInfo;
    }
